...
The first thing to do after setting up the environment is calibration. First, check that the encoders are working with 'cd src/Exoskeleton/test_scripts' 'python read2encoders.py'; then turn the right motor very low by hand (the left one can be a little higher, as the code uses the right one for synchronisation, but if you turn the left one too low during calibration, it will touch the screw because it calibrates by going down); then use 'odrivetool' and do 'odrv0.axis0.requested_state = AXIS_STATE_FULL_CALIBRATION_SEQUENCE' and 'odrv1. axis0.requested_state = AXIS_STATE_FULL_CALIBRATION_SEQUENCE'. You should hear a beep for both motors. If you do not hear the beep, check the cable connections and tighten the ones that move the most with a screwdriver.

Once calibration is complete, the codes to run are 1) 'ros2 run exo_control twoboards_odrive' 2) 'ros2 run exo_control admittancecontrol_box' 3) 'cd exo_control/exo_control' 'python tobii.py'
