...
DOT_SIZE = 60
OUT_MP4_NAME  = "custom_plot.mp4"
TARGET_FPS    = 30
SPEED_FACTOR  = 2.0   # 1.0 = tempo reale; >1 più veloce; <1 più lento
MP4_DPI       = 150
MP4_BITRATE   = 4000
...
def main():
    ...
    # --- PRE-PROCESSING: SHIFT (prima) + RESCALING (dopo) ---
    theta_w_raw = df_win[col_pos].to_numpy() if col_pos in df_win.columns else None

    theta_w_orig = None   # per traccia "orig" (shiftata ma non riscalata)
    theta_w_used = None   # quella che usi per modello e plot principale

    if theta_w_raw is not None:
        # 1) shift in radianti (valore dato in gradi)
        #    negativo = verso il basso
        theta_shift_rad = math.radians(SHIFT_THETA_DEG)
        theta_w_shifted = theta_w_raw + theta_shift_rad

        # salva la traccia pre-rescaling (già shiftata) per eventuale overlay
        theta_w_orig = theta_w_shifted.copy()

        # 2) rescaling affine ancorato al minimo -> THETA_MAX_TARGET (in radianti)
        th_min = float(np.nanmin(theta_w_shifted))
        th_max = float(np.nanmax(theta_w_shifted))
        ok_range  = (th_max > th_min + 1e-9)
        ok_target = (ALLOW_EXPANSION or (THETA_MAX_TARGET <= th_max))

        if ok_range and ok_target:
            s = (THETA_MAX_TARGET - th_min) / (th_max - th_min)
            theta_w_used = th_min + s * (theta_w_shifted - th_min)
        else:
            # nessun rescaling: usa solo lo shift
            theta_w_used = theta_w_shifted
    else:
        print("[ERRORE] theta_w non disponibile nel CSV/bag.", file=sys.stderr)
        sys.exit(1)


    # Modello da theta_w
    tau_w_model = compute_tau_w(theta_w_used) if theta_w_used is not None else None
    tau_b_model = compute_tau_box(theta_w_used) if theta_w_used is not None else None
    ...
    y_ass = tau_ass_nobox_appl if tau_ass_nobox_appl is not None else None
    ...
    # Costruisci timeline a fps fisso
    t_start = float(t_rel[0])
    t_end   = float(t_rel[-1])
    real_span = max(1e-9, t_end - t_start)                  # secondi reali della finestra
    anim_fps  = min(60.0, max(1.0, TARGET_FPS * SPEED_FACTOR))
    n_frames  = int(round(real_span * anim_fps)) + 1
    t_anim    = np.linspace(t_start, t_end, n_frames)

    # Mappa ogni frame all’indice dati più vicino (sottocampionamento)
    idx_per_frame = np.searchsorted(t_rel, t_anim, side='left')
    idx_per_frame = np.clip(idx_per_frame, 0, len(t_rel)-1)

    def update(i):
        k = int(idx_per_frame[i])
        t = t_rel[k]
        ball1.set_data([t], [y_pos[k]])
        if y_ass is not None:
            ball3.set_data([t], [y_ass[k]])
        return ball1, ball3

    ani = None
    if ENABLE_ANIMATION and n_frames > 1:
        ...
        try:
            import matplotlib as mpl
            # (opzionale se ffmpeg non viene trovato automaticamente)
            # mpl.rcParams['animation.ffmpeg_path'] = '/usr/bin/ffmpeg'

            writer = FFMpegWriter(
                fps=int(round(anim_fps)),
                codec='libx264',
                bitrate=MP4_BITRATE,
                extra_args=['-pix_fmt', 'yuv420p', '-movflags', '+faststart']
            )
            ani.save(mp4_path, writer=writer, dpi=MP4_DPI)
            print(f"[OK] Video salvato in: {mp4_path}")
        ...
        print("[INFO] Animazione disattivata o dati insufficienti: salto il salvataggio MP4.")
