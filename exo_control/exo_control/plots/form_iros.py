# -*- coding: utf-8 -*-
"""
Questionario HRC su scala Likert (0-4)
...
# === 1) INSERISCI DATI  ===
# ===========================
# Ogni riga = un partecipante. Ogni riga ha 13 valori (Q1..Q13) su scala 0..4.
...
def format_desc_cell(mean, std, vmin, vmax):
    if np.isnan(mean):
        return "--"
    return f"{mean:.2f}$\\pm${std:.2f} [{vmin:.0f}--{vmax:.0f}]"


def build_latex_desc_table(desc_df):
    lines = []
    lines.append(r"\begin{tabular}{lccc}")
    lines.append(r"\hline")
    lines.append(r" & EC1 & EC2 & EC3 \\")
    lines.append(r"\hline")
    for qi in range(1, NUM_Q + 1):
        q = f"Q{qi}"
        row = [q]
        for cond in ["NoExo", "ExoNoVision", "ExoVision"]:
            rec = desc_df[(desc_df["question"] == q) & (desc_df["condition"] == cond)]
            if len(rec) == 0:
                row.append("--")
            else:
                r = rec.iloc[0]
                row.append(format_desc_cell(r["mean"], r["std"], r["min"], r["max"]))
...
print("\n=== LaTeX table (0--4 Likert) ===")
print(build_latex_desc_table(desc))
...
# ============================================
# === 6b) PLOT LIKERT (DIVERGING STACKED)   ===
# ============================================

def plot_diverging_stacked(long_df, out_prefix="likert_diverging"):
    conds = ["NoExo", "ExoNoVision", "ExoVision"]
    scores = [0, 1, 2, 3, 4]
    colors = {
        0: "#9e2a2b",
    ...
    for cond in conds:
        ...
        qs = [q for q in [f"Q{i}" for i in range(1, NUM_Q + 1)] if len(sub[sub["question"] == q]) > 0]
        if not qs:
            continue
        fig, ax = plt.subplots(figsize=(10, 0.5 * len(qs) + 2), dpi=140)
        for yi, q in enumerate(qs):
            vals = sub[sub["question"] == q]["score"].astype(int)
            total = len(vals)
            if total == 0:
                continue
            counts = {s: int((vals == s).sum()) for s in scores}
            pct = {s: 100.0 * counts[s] / total for s in scores}
            # left (negative) side: 1 then 0
            left = 0.0
            for s in [1, 0]:
                w = pct[s]
                if w > 0:
                    ax.barh(yi, -w, left=left, color=colors[s], edgecolor="white", height=0.8)
                    left -= w
            # neutral centered on zero
            neu = pct[2]
            if neu > 0:
                ax.barh(yi, neu, left=-neu/2.0, color=colors[2], edgecolor="white", height=0.8)
            # right (positive) side: 3 then 4
            left = 0.0
            for s in [3, 4]:
                w = pct[s]
                if w > 0:
        ...
        ax.set_title(f"Diverging Likert Distribution ({cond})")
        # Legend
        handles = [plt.Rectangle((0, 0), 1, 1, color=colors[s]) for s in scores]
        labels = [str(s) for s in scores]
        ax.legend(handles, labels, title="Score", ncol=5, loc="lower center", bbox_to_anchor=(0.5, -0.15))
        plt.tight_layout()
        out_path = f"{out_prefix}_{cond}.png"
        plt.savefig(out_path, bbox_inches="tight")
        print(f"\nGrafico salvato: {out_path}")

plot_diverging_stacked(df_long)

# ============================================
# === 6c) BOX PLOTS + JITTERED POINTS      ===
# ============================================

def plot_boxplots_jitter(long_df, out_path="likert_boxplots.png"):
    qs = [f"Q{i}" for i in range(1, NUM_Q + 1)]
    conds_all = ["NoExo", "ExoNoVision", "ExoVision"]
    colors = {
        "NoExo": "#a8cfff",
        "ExoNoVision": "#3a86ff",
        "ExoVision": "#1d4ed8",
    ...
    for q in qs:
        if q == "Q1":
            ...
            conds = ["ExoNoVision", "ExoVision"]

        start = x
        for i, cond in enumerate(conds):
            vals = long_df[(long_df["question"] == q) & (long_df["condition"] == cond)]["score"]
            data.append(vals.values if len(vals) else [])
            positions.append(x + i * box_width)
            color_list.append(colors[cond])
        # center tick on group
        tick_pos.append(x + (len(conds) - 1) * box_width / 2.0)
        tick_labels.append(q)
        x = x + len(conds) * box_width + group_gap
    ...
    for pos, vals in zip(positions, data):
        ...
        ax.scatter(pos + jitter, vals, s=10, color="#333333", alpha=0.7, linewidths=0)
    ...
    ax.set_ylabel("Likert (1–4)", fontsize=26)
    ax.set_title("Distribution of Questionnaire Responses (Likert 1–4)", fontsize=28)
    ax.grid(axis="y", alpha=0.25)

    handles = [plt.Line2D([0], [0], color=colors[c], lw=8) for c in conds_all]
    ax.legend(
        handles,
        [r"EC1$_{Q}$", r"EC2$_{Q}$", r"EC3$_{Q}$"],
        loc="upper center",
...
plot_boxplots_jitter(df_long)

# ============================================
# === 6d) BOX PLOTS GRID (PREVIOUS STYLE)  ===
# ============================================

def plot_boxplots_grid_jitter(long_df, out_path="likert_boxplots_grid.png"):
    conds = ["NoExo", "ExoNoVision", "ExoVision"]
    qs = [q for q in [f"Q{i}" for i in range(1, NUM_Q + 1)] if len(long_df[long_df["question"] == q]) > 0]
    n = len(qs)
    cols = 4
    rows = int(np.ceil(n / cols)) if n else 1
    fig, axes = plt.subplots(rows, cols, figsize=(cols * 4.0, rows * 3.0), dpi=140, squeeze=False)
    rng = np.random.default_rng(0)
    for idx, q in enumerate(qs):
        ...
        for i, vals in enumerate(data, start=1):
            ...
            ax.scatter(i + jitter, vals, s=12, color="#2b6cb0", alpha=0.7, linewidths=0)
    ...
    fig.suptitle("Distribution of Questionnaire Responses (Likert 1–4)", y=1.02, fontsize=28)
    plt.tight_layout()
    plt.savefig(out_path, bbox_inches="tight")
    print(f"\nGrafico salvato: {out_path}")

plot_boxplots_grid_jitter(df_long)
