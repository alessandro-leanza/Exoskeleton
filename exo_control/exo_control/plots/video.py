...
DEFAULT_INPUT_BASENAME  = '/home/alessandro/Videos/icra/video_glasses_icra_task_NOT_seg1.mp4'           # senza estensione
DEFAULT_OUTPUT_BASENAME = '/home/alessandro/Videos/icra/video_glasses_icra_task_NOT_DONE_seg1.mp4'          # senza estensione; salveremo come .mp4
...
def _resolve_local_path(basename_or_path: str, default_exts=('.mp4', '.avi', '.mkv', '.webm')) -> str:
    ...
    """
    # 1) se esiste già così com'è (assoluto o relativo), usalo
    if os.path.exists(basename_or_path):
        return basename_or_path

    # 2) se non ha estensione, prova quelle note nella cartella dello script
    ...
    if ext == '':
        for e in default_exts:
            ...
            if os.path.exists(cand):
                return cand
        # prova anche nella stessa cartella con solo basename (magari è un named pipe/file senza estensione)
        cand = os.path.join(script_dir, basename_or_path)
        return cand
    else:
        # ha un'estensione ma non esiste: cerca nella cartella dello script
        cand = os.path.join(script_dir, basename_or_path)
        return cand
        ...
        if boxes is not None and len(boxes) > 0:
            for b in boxes:
                try:
                    ...
                    # filtro per-classe
                    min_req = MIN_CONF.get(label, GLOBAL_MIN_CONF)
                    if p < min_req:
                        continue
                    ...
                    # label (senza conf — aggiungila se ti serve)
                    label_text = label  # oppure f"{label} {p:.2f}"
